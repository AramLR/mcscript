...
generate = function (
  exp,
  oldFile,
  prjPath = oldFile.split("/").indexOf("function"),
  prjName = "noname"
) {
  let processed = {};
...
const parseCode = function (ast, oldFile, preOptions = null) {
  return new Promise(function (resolve, reject) {
    ...
    oldFile = oldFile.replace(/scripts\//, "function/");
    // let stream = InputStream(str)
    // let output = TokenStream(stream)
    // let ast = parse(output)
    // console.log(JSON.stringify(ast))
    let prjPath = oldFile.split("/").indexOf("function");
    let prjName = "noname";
    if (prjPath > -1) {
    ...
    if (
      oldFile.split("/").splice(-2).join("/") === "function/load.mcscript" ||
